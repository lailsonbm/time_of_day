...
If you had, you know: it sucks. Not anymore.
...
Cool. Just after a +rake db:migrate+, we can create trips and tell if the train will be
...
the canonical (and arbitrary) date +2000-01-01+. This is okay when you are comparing only attributes
...
because +Time#parse+ yields the current date (e.g. +2010-06-25 10:05') and of course it is not between 
+2000-01-01 10:00+ and +2000-01-01 10:10+. To accomplish what you were trying, you have two alternatives:
...
Sorry, both options sucks.

== How time_of_day handles the problem

There are many ways to fix this issue. Subclassing +Time+ would be certainly the most pure OO
...
though, is that Active Record time attributes now are always mapped to times of day:
...
At this time, the code work effortlessly:
...
Tested with Rails 3.0.0.beta4 on Ruby 1.9.2-preview3 and Ruby 1.8.7.

== Copyright

Copyright (c) 2010 Lailson Bandeira. See LICENSE for details.
